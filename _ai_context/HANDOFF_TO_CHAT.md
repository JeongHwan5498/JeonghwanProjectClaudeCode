...
fanding.kr `@sesang101` (세상학개론) 유료 멤버십 포스트를 자동 수집하여 투자 지식 베이스를 만드는 프로젝트.
...
**작동 방식**:
1. `python fanding_scraper.py` 실행
2. Firefox 브라우저가 열리고 fanding.kr 로그인 페이지로 이동
3. 사용자가 직접 네이버 소셜 로그인 완료
...
| 결정 | 이유 |
|------|------|
| 로그인은 수동으로 | 네이버 소셜 로그인 자동화 시 추가 인증 발생 |
...
1. **포스트 목록 수집 검증**: 실제 실행 시 포스트가 정상적으로 탐지되는지 확인
   ...
   - 현재 코드는 `/post/`와 `/@sesang101/` 패턴 둘 다 탐색하도록 수정됨
...
- OCR: Tesseract 5.4.0 (`C:\Program Files\Tesseract-OCR\tesseract.exe`), 한국어 팩 포함
- 프로젝트 경로: `C:\Users\user\Desktop\Claude Code\1. 세상학개론 프로젝트\`

---

## 핵심 파일

- `fanding_scraper.py` — 메인 스크래퍼
